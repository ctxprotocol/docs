...
| Tool | Cost | Use it to |
|------|------|-----------|
| `context_discover` | Free | Find which marketplace tools and methods exist before querying or executing |
...
- **Default to `context_query`.** It runs the full managed runtime and returns a grounded result for one fee. Omit `toolIds` for Auto Mode discovery and selection.
...
- **Use `context_discover` + `context_execute`** only when you want to drive exact method calls yourself: you already know the exact tool and method, or you need deterministic single calls with explicit spend control.
- **Call `context_discover` first** whenever you are unsure what is available. It is free and returns tool IDs, method signatures, and prices.
...
- Pass `toolIds` only to restrict the query to specific tools returned by `context_discover`.
- Pass `includeDeveloperTrace: true` when you need to debug why the runtime chose certain tools.

## context_execute: direct calls

1. Get `toolId`, `toolName`, and the argument shape from `context_discover` (the method signature lists required args; optional args end with `?`).
...
2. Optionally call `context_discover` to see what is available.
3. Call `context_query` with `responseShape: "evidence_only"` if you will write the answer, or the default shape for a ready answer.
4. If `capability_miss`, tell the user or retry with one of the suggested rewrites.
5. Present the grounded result and name the venue(s) used.
