...
A strong RFB unbundles a slice of a premium subscription into an answer an agent would naturally be asked. Before you build, write down the exact sentence a buyer would type into an agent to get this answer. If that sentence feels awkward, if the workflow is really someone scrolling a dashboard or scanning a feed, if a free LLM with browsing could plausibly fake the answer, or if the real buyer is already on an enterprise contract (Bloomberg, FactSet, 6sense) they will not leave for \$0.10 micropayments, the slice is not agent-native yet. Pick a different slice.

The pattern that works: prosumer subscriptions (CoinGlass, Glassnode, Hyblock, BamSEC) where real users already ask actual questions in natural language. The pattern that fails: enterprise terminal workflows where buyers scroll dashboards inside a contract they won't cancel.
</Warning>
...
| Earnings estimate revision momentum | `Query` or `both` | EE / revision screens | **No** (see `Future Categories`) |
| Insider cluster-buy intelligence | `Query` or `both` | insider screening workflow | **Yes** |
| Supply-chain shock exposure intelligence | `Query` | SPLC + manual risk research | **No** (see `Future Categories`) |
...
**High priority, with a sharper filter than before.** This category is strongest when you unbundle a workflow that a prosumer investor (FinTwit, small-cap community, independent analyst, value investor, short seller) would naturally ask as a question and pay to have answered with evidence. It does not work when the real buyer is a desk analyst on Bloomberg, FactSet, or Capital IQ, because those buyers will not switch to \$0.10 per answer.
...
- **Narrow developer-facing primitives** for normalizing painful public data (filings, ETF disclosures)

The classic analyst workflow lines (reverse DCF, earnings quality, comp valuation) still carry a stricter bar because free LLMs already produce a mediocre version.
...
Lines listed as `on ice` are the ones that survived the agent-native sanity check. Lines that failed it (ETF basis dislocation, earnings estimate revisions, Bloomberg SPLC clones, Cortellis clones, AI code-remediation) are not listed because they would not convert at \$0.10 per answer regardless of grant status.
</Note>
