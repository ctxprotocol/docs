...
{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Warning>
    **Batch 1 grant applications are closed.** We are focused on growing
    demand-side users so existing and new tools get paid usage. There is
    **no timeline for Batch 2** — we will reopen when marketplace demand
    justifies more funded supply.

    **Permissionless listing is still open:** [build and list your
    tool](https://ctxprotocol.com/contribute) anytime and keep **90% of
    usage fees** in perpetuity.
...
{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Note>
    **Batch 1 is closed.** Pool and tier details below are kept for
...
{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Note>
    Batch 1 applications are closed. The steps below are **archived for
    reference** when the next batch opens. To ship today without a grant,
    start at [ctxprotocol.com/contribute](https://ctxprotocol.com/contribute).
  </Note>
)}
