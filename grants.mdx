...
description: "Developer grants and contributor guidance — list permissionlessly and earn 90% USDC revenue share on the Context marketplace"
---

{/*
  BATCH TOGGLE — reopening grants for a new batch:
  1. Set GRANTS_ACCEPTING_APPLICATIONS = true (below)
  2. Google Form → Responses → turn ON "Accepting responses"
  Full revert checklist: changelog/2026-05-30-grants-batch-1-closed.mdx
*/}

export const GRANTS_ACCEPTING_APPLICATIONS = false;

export const GRANT_APPLICATION_FORM_URL =
  "https://docs.google.com/forms/d/e/1FAIpQLSfM702YEtiCK5o22bA6hbM7cke7psDbfx4t9-Oj7OsnPjDhtA/viewform";

{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Warning>
    <strong>Batch 1 grant applications are closed.</strong> We are focused on growing
    demand-side users so existing and new tools get paid usage. There is{" "}
    <strong>no timeline for Batch 2</strong>. We will reopen when marketplace demand
    justifies more funded supply.

    <strong>Permissionless listing is still open:</strong>{" "}
    <a href="https://ctxprotocol.com/contribute">build and list your tool</a> anytime and keep{" "}
    <strong>90% of usage fees</strong> in perpetuity.

    Applications already in review and approved Batch 1 projects are still
    honored on existing payment terms.
  </Warning>
)}
...
{GRANTS_ACCEPTING_APPLICATIONS ? (
  <CardGroup cols={2}>
    <Card title="Apply Now" icon="rocket" href={GRANT_APPLICATION_FORM_URL}>
  ...
  <CardGroup cols={2}>
    ...
    <Card title="List on Context" icon="rocket" href="https://ctxprotocol.com/contribute">
      Register your MCP endpoint on the permissionless marketplace.
    </Card>
  </CardGroup>
)}
...
{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Note>
    <strong>Batch 1 is closed.</strong> Pool and tier details below are kept for
    reference. New contributors can still list permissionlessly and earn
    revenue share.
  </Note>
)}
...
| **Authed CRM write-action primitive** | Composio / Zapier (one-CRM slice) | `Execute` + handshake | founder / GTM operator's agent | *(Execute primitive: create or update records in one CRM via OAuth handshake, not a generic gateway)* | Medium |

Not funded even when reopened: generic lead databases, CRM enrichment wrappers, "ZoomInfo for agents," "all-app" integration gateways, or enterprise-GTM-anchored lines. An authed action is fundable only as one narrow scoped write primitive behind a handshake, not as a broad Zapier-style proxy.
  </Accordion>
...
| **Ad-account audit + guarded budget/bid action** | Ryze / Adspirer (ad-ops copilots) | `Execute` + handshake | solo media buyer / small agency | *(Execute + handshake: read account performance, then a budget or bid change behind a spend-capped approval)* | Medium |
| **Authed social posting / scheduling action** | Composio / Zapier (one-app slice) | `Execute` + handshake | agent builder / social operator | *(Execute primitive: one app via OAuth handshake, e.g. post or schedule to X — not a 6,000-app gateway)* | Medium |

Not funded even when reopened: generic SEO suites, broad AI marketing wrappers, "browse this feed of ads" products, or "all-app" integration gateways. Money-moving or write actions are only fundable as one narrow scoped primitive behind a handshake (OAuth and, for spend, a budget envelope), never as a generic Zapier-style proxy.
  </Accordion>
...
When we reopen this category, it will fund two shapes: (1) tools that compress expensive developer toil for solo or small-team devs who have no Snyk / GHAS budget, and (2) narrow **agent-infrastructure `Execute` primitives** a coding agent calls directly and consumes raw. The second shape is the lane behind some of the best-selling MCP tools today (web fetch, extraction, component retrieval). It is fundable only when it isolates the specific paid capability, not when it is a generic "call any API" proxy.
...
| **JS-rendered web page → clean LLM-ready markdown** | Firecrawl | `Execute` | coding-agent / RAG pipeline builder | *(Execute primitive: anti-bot, JS-rendered fetch → portable markdown; the one slice devs actually pay Firecrawl for)* | Medium-High |
| **Structured extraction for one high-value site class** | Apify / Bright Data | `Execute` | scraping / data-pipeline dev | *(Execute primitive: typed records from one site type, not "scrape anything")* | Medium |
| **UI component retrieval primitive** | 21st.dev Magic | `Execute` | coding agent / frontend dev | *(Execute primitive: return a working component implementation by spec)* | Medium |

Not funded even when reopened: generic wrappers around GitHub, Datadog, code-search APIs, or AI code-remediation (Cursor and Claude Code already do this for free with repo context). A scrape or extraction primitive only fits if it isolates the specific paid capability (anti-bot rendering, clean markdown, one structured site class); a generic "fetch any URL" proxy is not fundable.
  </Accordion>
...
{!GRANTS_ACCEPTING_APPLICATIONS && (
  <Note>
    Batch 1 applications are closed. The steps below are{" "}
    <strong>archived for reference</strong> when the next batch opens. To ship
    today without a grant, start at{" "}
    <a href="https://ctxprotocol.com/contribute">ctxprotocol.com/contribute</a>.
  </Note>
)}
...
<Steps>
  ...
  <Step title="Submit Proposal">
    ...
    {GRANTS_ACCEPTING_APPLICATIONS && (
      <p>
        <a href={GRANT_APPLICATION_FORM_URL}>Submit Proposal</a>
      </p>
    )}
  </Step>
...
<CardGroup cols={2}>
  {GRANTS_ACCEPTING_APPLICATIONS ? (
    ...
    <Card title="Email Support" icon="envelope" href="mailto:grants@ctxprotocol.com">
      Questions about existing Batch 1 grants or permissionless listing.
    </Card>
  )}
...
{GRANTS_ACCEPTING_APPLICATIONS ? (
  ...
  <Card title="List Your Tool" icon="code" href="https://ctxprotocol.com/contribute">
    Ship permissionlessly and earn 90% of usage fees
  </Card>
)}
