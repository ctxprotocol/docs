...
<Info>
When you move from this toy example to real third-party APIs, add `_meta.rateLimit` hints to your tools. See [Tool Metadata](/guides/tool-metadata#rate-limit-hints).
</Info>
...
<Tip>
This toy tool has no parameters, but real tools should add `default` and `examples` to `inputSchema` properties. The AI reads these during orchestration to generate correct arguments — without them, it guesses from the description alone, leading to more retries. See [Build & List Your Tool](/guides/build-tools) for the full pattern.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Context Injection" icon="user" href="/guides/tool-metadata#context-injection">
...
<AccordionGroup>
  <Accordion title="Unauthorized error when calling tools">
