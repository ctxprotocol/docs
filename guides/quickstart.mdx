...
<Warning>
**Important: You cannot test `tools/call` locally with curl.** The `createContextMiddleware()` requires a valid JWT from the Context Platform. When you call `tools/call` without this JWT, you'll get `{"error":"Unauthorized"}`. This is expected behavior — it means your security middleware is working correctly.

To test your tools, you must:
1. Deploy to a public HTTPS URL
2. Register on the Context Marketplace
3. Call your tool through the Context app
</Warning>
...
<AccordionGroup>
  <Accordion title='{"error":"Unauthorized"} when calling tools' defaultOpen={true}>
    **This is the most common error.** It means your security middleware is working correctly.
    
    The `createContextMiddleware()` verifies that requests come from the Context Platform with a valid JWT. Without this JWT, `tools/call` will always return Unauthorized.
    
    **Causes:**
    - **Testing locally with curl** — You cannot test `tools/call` locally. Only `initialize` and `tools/list` work without auth.
    - **HTTP instead of HTTPS** — Context Platform requires HTTPS. Deploy to Railway, Vercel, or use a service like Caddy for local HTTPS.
    - **Not registered on marketplace** — Your tool must be registered at [ctxprotocol.com/contribute](https://ctxprotocol.com/contribute) before the platform will send authenticated requests.
    - **Wrong endpoint URL** — Double-check the URL you registered matches your deployed server.
    
    **Solution:** Deploy to HTTPS, register on the marketplace, then test through the Context app.
  </Accordion>
