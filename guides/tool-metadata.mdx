...
<Info>
SDK consumers can use **both** modes. Use `client.query.run()` when you want Context to be the librarian (curated intelligence, pay-per-response). Use `client.tools.execute()` when your agent is the librarian (raw data, per-call pricing, spending limits). See [TypeScript SDK](/sdk/reference) or [Python SDK](/sdk/python-reference).
</Info>
