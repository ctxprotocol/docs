...
Query mode now includes a managed `code_interpreter` tool. After the runtime calls your MCP methods, the interpreter can turn raw structured outputs into derived metrics, chart artifacts, metric tables, rolling statistics, correlations, drawdowns, and other post-processed evidence.
