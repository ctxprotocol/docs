---
title: "Tool Metadata"
description: "Configure _meta on your MCP tools to declare pricing, surface eligibility, rate-limit hints, and context injection requirements"
---
...
- **[Pricing & Surface Eligibility](#pricing--surface-eligibility)** — control which marketplace surfaces your methods appear on and set execute pricing
- **[Rate-Limit Hints](#rate-limit-hints)** — tell the planner and runtime how to pace calls to your API
...
## Pricing & Surface Eligibility

<Warning>
**This section determines whether your tool is visible on the Execute surface.** Methods without explicit `_meta.pricing.executeUsd` are **query-only** and will not appear in SDK-level Execute discovery. This is a hard platform gate, not a suggestion.
</Warning>

### Why This Matters

Context runs **one marketplace with two surfaces**:

| Surface | Who is the librarian? | Billing | What you get |
|---------|------------------------|---------|---------------------|
| **Query** | Context runtime (used via the app **or** `client.query.run()` in the SDK) | Pay-per-response (~$0.10) | Curated AI-synthesized answer from up to 100 MCP calls |
| **Execute** | Your app/agent (used via `client.tools.execute()` in the SDK) | Per method call (~$0.001) | Raw structured data with session spend visibility |

<Info>
SDK consumers can use **both** surfaces. Use `client.query.run()` when you want Context to be the librarian (curated intelligence, pay-per-response). Use `client.tools.execute()` when your agent is the librarian (raw data, per-call pricing, session budgets). See [TypeScript SDK](/sdk/reference) or [Python SDK](/sdk/python-reference).
</Info>

Execute pricing is typically **~1/100 of listing response price** because a single Query response can invoke up to 100 method calls internally. When developers pay per-execute-call, they need proportionally lower prices.
...
| Field | Type | Purpose |
|---|---|---|
| `surface` | `"answer"` \| `"execute"` \| `"both"` | Which marketplace surface(s) this method is eligible for |
...
- Methods **with** `_meta.pricing.executeUsd` → visible in both Query and Execute surfaces (depending on `surface` field)
...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Stream real-time trade data (Execute surface only)",
    _meta: {
      surface: "execute",
      queryEligible: false,
...
| `queryEligible` | `true` |
| `latencyClass` | `"slow"` |
| `pricing.executeUsd` | Not set → **query-only in Execute surfaces** |

<Info>
To enable Execute surface visibility for a legacy tool, either set a default execute price in the Developer Tools page or add `_meta.pricing.executeUsd` to your MCP server code and click "Refresh Skills".
</Info>

### Reference Implementation

The [Coinglass contributor server](https://github.com/ctxprotocol/sdk/tree/main/examples/server/coinglass-contributor) demonstrates all `_meta` capabilities in production:
