...
<Info>
**Running on hobby/free API tiers?** Publish `_meta.rateLimit` hints on your tools so planner + runtime can pace safely. See [Tool Metadata](/guides/tool-metadata#rate-limit-hints).
</Info>
...
<Info>
**Earnings Model**: You earn **90%** of usage fees. Set a **listing response price** (for example, \$0.01/response) for Query surface usage, and optionally set **method-level execute pricing** for Execute surface calls. Payment is settled automatically after successful execution flows.
</Info>
...
Context's agentic pipeline uses a **planning LLM** that reads your `outputSchema` to generate code that processes your tool's response. If your schema is vague, the LLM has to *guess* property names — and it will guess wrong. This triggers an automatic **self-healing retry loop** that:
...
<Warning>
**The planning LLM defaults to camelCase** (standard JavaScript convention). If your API returns `snake_case` and your `outputSchema` doesn't document the exact property names, the LLM will generate `data.exchangeName` when your API returns `data.exchange_name` — triggering a retry every time.
</Warning>

```typescript
// ❌ Bad: Vague schema — LLM must guess property names
outputSchema: {
  type: "object",
  properties: {
...
1. **No database bloat** — URLs are small strings; base64 images are 100x larger
2. **Faster responses** — No large payloads to transfer
3. **Standard web pattern** — This is how Slack, Discord, and every major chat platform works
4. **You control caching** — Set your own CDN caching policies
5. **You control availability** — Your images, your infrastructure, your reliability
...
Context runs one marketplace with two surfaces. Your pricing determines where your tool appears:

| Price | Use Case |
|-------|----------|
| **Listing response price: \$0.00** | Free listing usage in Query surface |
| **Listing response price: \$0.01+** | Paid Query usage (per response turn) |
| **Default execute price (optional)** | Convenience input that fans out to each method's `_meta.pricing.executeUsd` |

<Note>
Query fees are paid **once per chat turn**. The runtime may call methods up to 100 times within that turn as a safety-capped orchestration limit.
</Note>

<Warning>
**Execute Surface Gating Rule:** Methods without explicit `_meta.pricing.executeUsd` are **invisible** on the Execute surface. SDK consumers cannot discover or call them. This is a hard platform gate — if you want SDK-level execute revenue, you must set execute pricing.

Set a default execute price in the contribute form (fans out to all methods), or declare `_meta.pricing.executeUsd` per method in your MCP server code. See [Tool Metadata](/guides/tool-metadata#pricing--surface-eligibility) for the full metadata reference.
</Warning>

<Tip>
Execute pricing is usually much lower than listing response pricing because it is charged per method call. A common starting point is approximately **1/100 of your listing response price** (e.g., \$0.10 response → \$0.001 per execute call).
</Tip>
...
Your MCP Tool is now instantly available on the decentralized marketplace. Query users can discover answer-safe methods, and execute clients can call explicitly execute-priced methods with session budgets.
...
<Warning>
**Critical**: MCP tool execution on the Context platform has a **~60 second timeout**. This is intentional — it shapes the marketplace toward high-quality data products.
</Warning>

### Where the Timeout Comes From

The timeout is enforced by the **platform infrastructure** (and in standard MCP setups like Claude Desktop, by the LLM client itself). When your tool is called, the system waits for a response — if it doesn't arrive in ~60 seconds, execution fails.
...
<Note>
This isn't an MCP protocol limitation — SSE connections can stay open indefinitely. The timeout exists at the application layer (LLM clients, API gateways, platform infrastructure) and serves as a **quality forcing function**.
</Note>

### Why the Timeout Is Actually Good

The timeout isn't a bug — it's a feature that forces data brokers to build **actual products** instead of raw data access.

| Raw Access (❌ Bad Product) | Data Broker Product (✅ Good Product) |
|----------------------------|--------------------------------------|
...
<AccordionGroup>
  <Accordion title="Tier 1: Real-time Queries (< 60s)" icon="bolt">
    Perfect for MCP — works great today:
  ...
  <Accordion title="Tier 2: Pre-computed Insights (instant)" icon="database">
    **This is the REAL product** — where data brokers add massive value:
  ...
  <Accordion title="What if my analysis takes longer than 60 seconds?" icon="clock">
    ...
    **The pattern**: Run your heavy analysis offline (cron jobs, scheduled tasks), store the results in your own database, and serve them instantly through your MCP tool.
    ...
    ```typescript
    // ❌ BAD: Long-running analysis at request time
    { name: "analyze_all_wallets", returns: "timeout after 60s" }
    
    // ✅ GOOD: Pre-computed results served instantly
    { name: "get_smart_money_wallets", returns: "instant response from your DB" }
    ```
    
    This is exactly how Bloomberg, Nansen, and Arkham work — the value is in the **curation and pre-computation**, not raw data access.
  </Accordion>
</AccordionGroup>
...
```typescript
// ❌ BAD: Raw SQL tool (timeout-prone, no moat)
{
  name: "run_sql",
...
// ✅ GOOD: Pre-computed insight tools
{
...
<Tip>
**The value you create**: Your data brokers should be selling "Nansen-as-a-service" and "Arkham-as-a-service" — not raw SQL access. The timeout forces this quality bar.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Hummingbot Market Intel" icon="python" href="https://github.com/ctxprotocol/sdk-python/tree/main/examples/server/hummingbot-contributor">
    Multi-exchange market data (6 tools) — prices, order books, funding rates, trade impact analysis
  </Card>
</CardGroup>
