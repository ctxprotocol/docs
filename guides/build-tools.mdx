...
<Steps>
  ...
  <Step title="Optimize and Ship">
    **Before you start:** Make sure your wallet has a small balance (a few dollars is enough). The optimization process makes real query calls through the Context platform to validate your tool's responses. If your wallet is empty, the librarian cannot complete the agentic flow and validation will fail. Deposit funds at [Developer Dashboard](https://ctxprotocol.com/developer).
    ...
    - Pushes the description directly to your listing via the SDK, with no manual copy-paste
...
Context's agentic pipeline uses an **iterative execution model** that reads your `outputSchema` to decide how to call tools, interpret results, and recover from mistakes. If your schema is vague, the runtime has to *guess* property names, and it will guess wrong. This triggers inline recovery that:
...
**The execution model defaults to camelCase** (standard JavaScript convention). If your API returns `snake_case` and your `outputSchema` doesn't document the exact property names, the runtime may infer `data.exchangeName` when your API returns `data.exchange_name`, triggering recovery every time.
...
For third-party APIs with strict quotas, publish pacing hints via `_meta.rateLimit`. These hints influence both metadata-scout pacing and iterative-execution call budgets. See [Tool Metadata](/guides/tool-metadata#rate-limit-hints). Reference implementation: [Coinglass contributor server](https://github.com/ctxprotocol/sdk/tree/main/examples/server/coinglass-contributor).
  </Accordion>
</AccordionGroup>
