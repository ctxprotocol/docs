...
The platform reads these fields during orchestration to generate correct arguments for your methods. In the internal `deep-heavy` Scout lane, they can also determine whether the runtime can form a safe bounded read-only pre-plan probe before planning. In `deep-light`, the same fields still improve metadata-first planning without probing. Without them, the AI must guess valid values from the description alone, leading to fewer safe probes, more retries, and lower first-pass success rates.
...
For third-party APIs with strict quotas, publish pacing hints via `_meta.rateLimit`. These hints influence both planner pacing and bounded pre-plan probe caps. See [Tool Metadata](/guides/tool-metadata#rate-limit-hints). Reference implementation: [Coinglass contributor server](https://github.com/ctxprotocol/sdk/tree/main/examples/server/coinglass-contributor).
  </Accordion>
</AccordionGroup>
...
<Steps>
  <Step title="Manual: Test in the Chat App">
    Enable **Developer Mode** in Settings → Developer Settings. In the right sidebar, turn off **Auto mode** and select your tool. Send a message and check the **Developer Logs** preview card at the bottom of the response for errors. See the [Debugging Guide](/guides/troubleshooting#using-developer-mode-for-debugging) for details.
  </Step>
  <Step title="Manual: Test via the SDK">
    ...
    - **Query mode:** `client.query.run({ query: "...", tools: ["your-tool-id"], queryDepth: "deep", includeDeveloperTrace: true })` — validates your tool produces meaningful answers through the agentic pipeline
  ...
  <Step title="Automated: Deep Validation System Prompt">
    For a comprehensive QA pass, use the **[Deep Validation System Prompt](https://github.com/ctxprotocol/sdk/blob/main/docs/mcp-contributor-deep-validation-system-prompt.md)** — a reusable prompt you can give to any coding agent (Cursor, Claude, etc.). It runs the full validation workflow:

    - Checks your server against the [Context Protocol docs](https://docs.ctxprotocol.com) for compliance
    - Tests both Query mode and Execute mode through the SDK with developer traces
    - Generates a marketplace listing (name, description, "Try asking" prompts)
...
**Critical**: MCP tool execution on the Context platform has a **~60 second timeout per call**. This applies to both Query and Execute mode. It shapes the marketplace toward high-quality data products — but what "high-quality" means depends on which mode you're designing for.
