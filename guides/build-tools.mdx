...
The platform reads these fields when generating arguments for your methods, when grounding clarifications, and when selecting the right method for a query. Without strong schema hints, the runtime has to guess valid values from the description alone, leading to more retries and lower first-pass success rates.
...
Context's managed runtime reads your `outputSchema` to decide how to call tools, interpret results, and recover from mistakes. If your schema is vague, the runtime has to *guess* property names, and it will guess wrong. Each guess triggers a retry that:
...
The runtime retries likely code/data-shape issues. Infrastructure failures (rate limits, auth failures, upstream timeouts) are treated as non-healable for that turn.
