...
| `favoritesOnly` | `boolean` | No | Override the account-level Favorites-Only Auto Mode for this request |
| `agentModelId` | `string` | No | Main librarian agent model ID (default: `glm-5.2-model`) |
...
Import `AGENT_MODEL_IDS` and `DEFAULT_AGENT_MODEL_ID` from `@ctxprotocol/sdk` to see the current supported slugs. The default is `glm-5.2-model`.
</Note>
