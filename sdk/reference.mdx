...
<Note>
`query.run()` uses the streaming path internally and has a default client stream timeout of 600 seconds (configurable via `streamTimeoutMs`). Non-streaming SDK requests default to 300 seconds. The hosted compute ceiling is 1800 seconds on every query path — sync, async jobs, and MCP. For complex chart-heavy requests, use `runOrPoll()` or async jobs.
</Note>
...
console.log(answer.response);
console.log(answer.computedArtifacts);
```

The defaults are agent-friendly: status is checked every 5 seconds over plain HTTP (HTTP polls cost no model tokens — model *turns* do), and the client waits up to 31 minutes, slightly beyond the 1800-second hosted compute ceiling. Only pass `pollOptions` if you need a shorter client-side wait.

Use `run()` for short interactive queries that should finish inside your terminal/tool timeout. Use `runOrPoll()` when query time is uncertain or when every SDK call costs an LLM inference turn.
...
<Info>
Reference implementation: [Normalized Data Provider](https://github.com/ctxprotocol/sdk/tree/main/examples/server/normalized-data-provider) — per-method `_meta.pricing` and `_meta.rateLimit` in production.
</Info>
