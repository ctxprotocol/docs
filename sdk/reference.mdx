...
console.log(answer.orchestrationMetrics); // Optional orchestration diagnostics
```
...
`includeDeveloperTrace` and `orchestrationMetrics` are optional diagnostic surfaces. Their inner fields are typed but may evolve across rollouts as the managed runtime changes, so treat them as debugging signals rather than a stable execution contract.
</Info>
...
interface QueryResult {
  ...
  durationMs: number;
  data?: unknown;                       // Raw fetched tool data (includeData=true; default for evidence_only)
