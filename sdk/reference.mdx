...
| `debugScoutDeepMode` | `"deep-light" \| "deep-heavy"` | No | Development/testing only internal deep lane override |
| `idempotencyKey` | `string` | No | Optional idempotency key (UUID recommended) |
...
Supports the same options as `run()` (`tools`, `modelId`, `includeData`, `includeDataUrl`, `includeDeveloperTrace`, `queryDepth`, `debugScoutDeepMode`, `idempotencyKey`).
