...
Supports the same options as `run()` (`tools`, `favoritesOnly`, `answerModelId`, `responseShape`, `includeData`, `includeDataUrl`, `includeDeveloperTrace`, `queryDepth`, `debugScoutDeepMode`, `idempotencyKey`).
