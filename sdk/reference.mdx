...
<Info>
**Which should I use?** Use **Query** when you want a curated answer — Context is the librarian and handles answer-safe tool discovery/orchestration (up to 100 MCP calls per query turn). Use **Execute** when your app/agent is the librarian and you want explicit method pricing + session-budgeted spend control.
</Info>
...
Run an agentic query. The server discovers answer-safe tools, executes the full pipeline (up to 100 MCP calls per query turn as a runtime safety cap), applies model-aware mediator/data budgeting, and returns an AI-synthesized answer. Query billing remains pay-per-response with deferred settlement.
