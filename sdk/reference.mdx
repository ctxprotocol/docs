...
<Info>
**Which should I use?** Use **Query** when you want a curated answer — Context is the librarian and handles answer-safe tool discovery/orchestration (up to 100 MCP calls per response turn). Use **Execute** when your app/agent is the librarian and you want explicit method pricing + session-budgeted spend control.
</Info>

<Note>
Mixed listings are first-class: one listing can expose methods to both surfaces. Methods without explicit execute pricing remain discoverable for Query but are excluded from strict Execute discovery when `requireExecutePricing=true`.
</Note>
...
// Query surface (pay-per-response)
...
Search for tools matching a query string, or pass an options object for surface-aware filtering.
...
| `surface` | `"answer" \| "execute" \| "both"` | No | Method surface filter |
...
|-----------|------|----------|-------------|
| `limit` | `number` | No | Maximum results to return |
| `options` | `Omit<SearchOptions, "query" \| "limit">` | No | Optional mode/surface filters |
...
Execute a single tool method. Execute calls can run inside a session budget (`maxSpendUsd`) for deferred batch settlement.
...
Run an agentic query. The server discovers answer-safe tools, executes the full pipeline (up to 100 MCP calls per response turn as a runtime safety cap), applies model-aware mediator/data budgeting, and returns an AI-synthesized answer. Query billing remains pay-per-response with deferred settlement.
...
interface McpTool {
  ...
  _meta?: McpToolMeta;                     // surface/eligibility/pricing/context metadata
...
2. **Execute mode (`client.tools.execute`)** accrues per-call method spend into execute sessions and flushes in deferred batches
