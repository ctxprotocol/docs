...
Since SDK 0.21.0, `run()` is backed by the durable job path (`start()` + `poll()`), so a single call reliably covers the full 1800-second hosted compute ceiling and survives transient connection drops. Pass `QueryPollOptions` as an optional second argument to shorten the client-side wait.
...
Since 0.21.0, `run()` and `runOrPoll()` share the same durable job-backed path, so either works for uncertain query times. `runOrPoll()` remains the explicit name for readers of agent code.
