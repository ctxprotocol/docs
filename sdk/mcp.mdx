...
Some marketplace questions are slow: multi-tool analysis, chart generation, long historical windows, and large data exports can exceed an MCP host's direct tool-call window. Context handles this with durable query jobs.
...
<Tip>
For Cursor specifically, keep the loop simple: one `context_query` or `context_query_start`, then `context_query_status`. If the status tool is not visible in Cursor's MCP tool list, refresh/restart the MCP server rather than falling back to file searches or shell polling.
</Tip>
