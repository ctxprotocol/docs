...
<Tip>
For natural-language questions, start with `context_query` and answer from that result. It already performed managed discovery, execution, grounding, and chart/artifact generation. Avoid repeated `context_discover` calls or a follow-up `context_execute` verification pass unless you need raw method output, exact rows, or pinned tools.
</Tip>
