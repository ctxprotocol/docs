...
The ctxprotocol MCP server exposes three tools (`context_query`, `context_execute`, `context_discover`). A short **Agent Skill** teaches your coding agent (Cursor, Claude Code, or any harness that supports Agent Skills) how to choose between them, when to ask for `evidence_only` vs `answer_with_evidence`, how to control spend, and how to handle capability misses.
