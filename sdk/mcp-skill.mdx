...
| `context_query_poll` | Free | Wait inside one MCP tool call while the backend librarian runtime continues tool calls, code-interpreter work, chart generation, and synthesis |
| `context_query_status` | Free | Check a `jobId` status once |
...
- **Long jobs:** if `context_query` returns `jobId`, call `context_query_poll` with that exact ID until it returns `completed` or `failed`. Do not start another paid query for the same prompt.
...
- For long jobs, `structuredContent.jobId` means the server is still working. Call `context_query_poll`; do not inspect local files or fetch blobs until the job is terminal.
...
- `jobId` means the query is still running asynchronously. Call `context_query_poll`; do not re-run `context_query`.
...
3. If the result returns `jobId`, call `context_query_poll` until terminal.
