...
The server discovers answer-safe tools, executes the full agentic pipeline (up to 100 MCP calls per query turn), handles retries and completeness checks, and returns a curated answer — all for one flat fee. See the [TypeScript SDK Reference](/sdk/reference#query-pay-per-response) or [Python SDK Reference](/sdk/python-reference#query-pay-per-response) for the full API.
