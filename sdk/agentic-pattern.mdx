...
The most powerful way to use the Context SDK is to **let your LLM do the driving**. Context provides one marketplace with two surfaces (Query + Execute), and there are two primary approaches:
...
The server discovers answer-safe tools, executes the full agentic pipeline (up to 100 MCP calls per response turn), handles retries and completeness checks, and returns a curated answer — all for one flat fee. See the [TypeScript SDK Reference](/sdk/reference#query-pay-per-response) or [Python SDK Reference](/sdk/python-reference#query-pay-per-response) for the full API.

### Option 2: Build Your Own Loop

If you want full control over tool selection, argument construction, and result synthesis, follow the **Discovery → Schema → Execution** loop below (typically on the Execute surface with session budgets).
...
For deterministic execution pipelines, use Execute-surface filters:
