...
Run an agentic query. The server applies discovery-first orchestration (`discover/probe -> plan-from-evidence -> execute -> bounded fallback`) with up to 100 MCP calls per response turn as a runtime safety cap, then returns an AI-synthesized answer. On the internal `deep-heavy` Scout lane, the runtime may perform a tiny bounded set of deterministic read-only pre-plan tool probes when safe; `deep-light` stays metadata-first, and `fast` remains one-shot biased. Query billing is pay-per-response with automatic payment after delivery.
...
| `debug_scout_deep_mode` | `"deep-light" \| "deep-heavy"` | No | Development/testing only internal deep lane override |
| `idempotency_key` | `str` | No | Optional idempotency key (UUID recommended) |
...
Inside `deep`, `deep-light` stays metadata-first while `deep-heavy` enables bounded Scout probing when the selected tool metadata is too weak to trust on its own.
