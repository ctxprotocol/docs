...
Since SDK 0.21.0, `run()` is backed by the durable job path (`start()` + `poll()`), so a single call reliably covers the full 1800-second hosted compute ceiling and survives transient connection drops. Pass optional `interval_ms` / `timeout_ms` keyword arguments to shorten the client-side wait.
...
Since 0.21.0, `run()` and `run_or_poll()` share the same durable job-backed path, so either works for uncertain query times. `run_or_poll()` remains the explicit name for readers of agent code.
