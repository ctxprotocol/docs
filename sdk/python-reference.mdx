...
<Info>
**Which should I use?** Use **Query** when you want a curated answer — Context is the librarian and handles answer-safe discovery/orchestration (up to 100 MCP calls per query turn). Use **Execute** when your app/agent is the librarian and you want explicit method pricing with session-budgeted spend control.
</Info>

<Note>
Mixed listings are first-class: one listing can expose methods to both surfaces. Methods without explicit execute pricing remain query-only until pricing metadata is added.
</Note>
...
Search for tools matching a query string, with optional surface-aware filters.
...
| `surface` | `"answer" \| "execute" \| "both"` | No | Optional method surface filter |
...
Execute a single tool method. Execute calls can run inside a session budget (`max_spend_usd`) for deferred batch settlement.
...
class Tool(BaseModel):
    ...
    name: str
    description: str
    price: str  # listing-level query price metadata
    category: str | None
...
class McpTool(BaseModel):
    ...
    meta: dict[str, Any] | None  # alias: "_meta" (surface/eligibility/pricing/context)
