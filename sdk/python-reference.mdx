...
If `response_shape` is `evidence_only`, Context skips the extra prose synthesis layer, but the librarian agent still runs to fetch, compute, and ground the result.

Import `AGENT_MODEL_IDS` and `DEFAULT_AGENT_MODEL_ID` from `ctxprotocol` to see the current supported slugs. Omit `agent_model_id` to use the managed default (`DEFAULT_AGENT_MODEL_ID`).
</Note>
...
<Note>
`query.run()` uses the streaming path internally and has a default client stream timeout of 600 seconds (configurable via `stream_timeout_seconds`). Non-streaming SDK requests default to 300 seconds. The hosted compute ceiling is 1800 seconds on every query path — sync, async jobs, and MCP. For complex chart-heavy requests, use `run_or_poll()` or async jobs.
</Note>
...
print(answer.response)
print(answer.computed_artifacts)
```

The defaults are agent-friendly: status is checked every 5 seconds over plain HTTP (HTTP polls cost no model tokens — model *turns* do), and the client waits up to 31 minutes, slightly beyond the 1800-second hosted compute ceiling. Only pass `interval_ms` / `timeout_ms` if you need a shorter client-side wait.

Use `run()` for short interactive queries that should finish inside your terminal/tool timeout. Use `run_or_poll()` when query time is uncertain or when every SDK call costs an LLM inference turn.
