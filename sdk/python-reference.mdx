...
Run an agentic query. The server applies discovery-first orchestration (`discover/probe -> plan-from-evidence -> execute -> bounded fallback`) with up to 100 MCP calls per response turn as a runtime safety cap, then returns an AI-synthesized answer. Query billing is pay-per-response with automatic payment after delivery.
...
- `auto`: server routes to either `fast` or `deep` using query intent and selected tool complexity.
