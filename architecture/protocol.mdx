...
<Note>
For the detailed server-managed flow across `/api/v1/query` and `/api/chat`, including lane routing (`fast` / `deep`), clarification, planning, synthesis, and the historical legacy deep aliases accepted in debug surfaces, see [Query and Chat Agentic Flow](/architecture/query-chat-agentic-flow).
</Note>
...
**Query** is for intelligence — you ask a natural-language question, and the server handles tool discovery, multi-tool orchestration (up to 100 MCP calls per response turn), targeted self-healing retries, completeness checks, and synthesis. One flat fee, with a response contract that can stay backward-compatible (`answer`) or return structured evidence packages (`answer_with_evidence`, `evidence_only`). This is the same agentic pipeline that powers the client app.
