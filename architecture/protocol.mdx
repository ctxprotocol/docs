...
<Note>
For the detailed server-managed flow across `/api/v1/query` and `/api/chat`, including lane routing (`fast` / `deep`), metadata scout, clarification, iterative execution, synthesis, and the historical legacy deep aliases accepted in debug surfaces, see [Query and Chat Agentic Flow](/architecture/query-chat-agentic-flow).
</Note>
...
<Steps>
  ...
  <Step title="Select">
    Chooses the lane and method universe for this request
  </Step>
  <Step title="Metadata Scout">
    Builds the execution shortlist and ambiguity pool from tool metadata
  </Step>
  <Step title="Clarify If Needed">
    Returns `clarification_required`, auto-resolves the recommended option, or returns `capability_miss`
  </Step>
  <Step title="Iterative Execute">
    Runs the managed tool loop with inline recovery, rediscovery, and verification
  </Step>
  <Step title="Synthesize">
    Combines verified results into a coherent answer and streams it to the user
  </Step>
  <Step title="Settle">
...
- Infrastructure-style failures (for example: `429`, auth failures, upstream timeout/unavailable responses) are treated as non-healable for that turn and skip inline recovery loops.
- The iterative executor still retries likely code/data-shape issues (property mismatch, parsing errors) when a recovery attempt can improve the answer.
- If verification indicates a capability or tier constraint (for example: required metric is unavailable on the current plan), the platform avoids repeated same-tool loops and returns best-effort output with limitations or a structured `capability_miss`.
- Isolated nulls are handled conservatively: a single non-critical null field (especially when outputs include capability-limit signals) does not trigger repeated recovery loops.
- Tool contributors can publish pacing hints in `_meta.rateLimit` / `_meta.rateLimitHints` (`maxRequestsPerMinute`, `cooldownMs`, `maxConcurrency`, `supportsBulk`, `recommendedBatchTools`, `notes`), which are surfaced to metadata-scout and iterative-execution logic.
...
| **Query** (`client.query.run()`) | `/api/v1/query` | Pay-per-response | Deferred (post-response) | Managed librarian output: `answer`, `answer_with_evidence`, or `evidence_only` from the full agentic pipeline |
...
**Query** is for intelligence — you ask a natural-language question, and the server handles discovery, selection, metadata scout, clarification if needed, iterative multi-tool execution (up to 100 MCP calls per response turn), synthesis, and settlement. One flat fee, with a response contract that can stay backward-compatible (`answer`) or return structured evidence packages (`answer_with_evidence`, `evidence_only`). This is the same managed runtime that powers the client app.
