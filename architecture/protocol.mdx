...
All value flows through `ContextRouter.sol` on Base, with settlement timing depending on mode:

- **Query mode (`/api/v1/query`)** uses deferred settlement after the response is delivered.
- **Execute mode (`/api/v1/tools/execute`)** currently settles before the MCP call (single-call pay-per-request flow).
...
1. User spending cap is checked
2. Payment is executed
3. MCP tool call runs once and returns raw output

<Info>
**Key benefit (Query mode):** Users never wait for on-chain transactions. They get their response immediately, and settlement happens asynchronously. Tool fees are waived if execution fails — users only pay for successful results.
</Info>
...
- Implementation guide: [Rate-Limit Metadata](/guides/rate-limit-metadata).
...
|------|----------|---------------|-------------------|--------------|
| **Execute** (`client.tools.execute()`) | `/api/v1/tools/execute` | Pay-per-request | Pre-execution (single-call flow) | Raw data from a single tool call |
