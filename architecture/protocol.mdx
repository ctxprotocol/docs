...
| Query surface | Pay-per-response (~\$0.10), answer-safe method gating, up to 100 MCP calls per turn, deferred settlement |
| Execute surface | Per-call method pricing (~\$0.001) inside execute sessions, deferred batch settlement |
| Unpriced execute methods | **Query-only** — invisible on Execute surface until `_meta.pricing.executeUsd` is set |
...
<Warning>
**Execute surface gating:** Methods without explicit `_meta.pricing.executeUsd` do not appear in SDK-level Execute discovery. They remain usable for Query only. Contributors must set execute pricing (either via the contribute form or in MCP server `_meta`) to unlock Execute surface visibility. See [Tool Metadata](/guides/tool-metadata#pricing--surface-eligibility) for configuration details.
</Warning>
