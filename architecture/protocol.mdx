...
description: "How the dual-surface marketplace, settlement layer, and safety model work together"
---

## Overview

Context is built on a **code-execution paradigm**. Instead of rigid "tool calling" schemas, the runtime can plan and execute tool interactions using typed MCP metadata, then settle economics according to the active surface.

The architecture consists of four layers:

1. **The Marketplace** — Supply of tools from developers
2. **The Agent** — Demand from users and developers across Query + Execute surfaces
...
| **Surface** | Where a method is eligible: `answer`, `execute`, or `both` |

### How It Works

1. You build an MCP server exposing your data/APIs
2. You register it as an MCP listing with a query price and optional method-level execute pricing metadata
3. Query runtime selects answer-safe methods; Execute runtime selects explicitly priced execute methods
...
Context runs one marketplace with two distinct surfaces:
...
All value flows through `ContextRouter.sol` on Base, with surface-specific settlement semantics:

- **Query mode (`/api/v1/query`)** keeps deferred settlement after the response is delivered.
- **Execute mode (`/api/v1/tools/execute`)** uses execute sessions (`maxSpendUsd`) with deferred batch settlement flushes.
...
<Info>
**Key benefit:** Both surfaces avoid blocking users on on-chain settlement. Query responses and execute results return first, then settlement is handled asynchronously. Failed executions do not silently overcharge successful-call economics.
</Info>
...
| Limit | Value | Purpose |
|-------|-------|---------|
| Max calls per query turn | 100 | Prevent runaway loops in Query runtime |
| VM timeout | 5000ms | Code execution limit |
| **MCP execution timeout** | **~60 seconds** | Encourages pre-computed products |

Every query-turn MCP call increments an internal counter. Once `executionCount >= 100`, the platform throws an error for that query turn.
...
| Query surface | Pay-per-response, answer-safe method gating, deferred settlement |
| Execute surface | Per-call method pricing inside execute sessions, deferred batch settlement |
| Unpriced execute methods | Query-only until execute pricing metadata is set |
...
The SDKs expose two distinct surfaces, reflecting Context's dual role as both a response marketplace and an execution marketplace:

| Mode | Endpoint | Payment Model | Settlement Timing | What You Get |
|------|----------|---------------|-------------------|--------------|
| **Execute** (`client.tools.execute()`) | `/api/v1/tools/execute` | Per execute call | Execute session accrual + deferred batch flush | Raw data + explicit spend envelope |
